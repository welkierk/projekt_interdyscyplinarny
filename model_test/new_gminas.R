...
for(gmina in all_gminas) {
  ...
  gmina <- gmina %>%
    select(-'Województwo') %>% # select(-'WojewÃ³dztwo') %>% if doesn't work
...
ourDevNotDevResults$gmina <- substr(ourDevNotDevResults$gmina,1,nchar(ourDevNotDevResults$gmina)-4)
ourDevNotDevResults$gmina <- stri_trans_general(str = ourDevNotDevResults$gmina, id = "Latin-ASCII")
ourDevNotDevResults <- ourDevNotDevResults[,-c(2,4)]
ourDevNotDevResults <- ourDevNotDevResults %>% distinct(gmina, .keep_all=TRUE)

write.csv2(ourDevNotDevResults, "../model_z_danymi_ze_zdj/klasy.csv", row.names = FALSE)
